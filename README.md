...
Written in TypeScript and compiled to ES6 with inline source map & source. See [@selfage/tsconfig](https://www.npmjs.com/package/@selfage/tsconfig) for full compiler options. Provides message descriptor definitions and utils to assist runtime processing. A message descriptor is typically generated by using `@selfage/cli`.

The term "message" stands for data class, inspired from Google's protobuf. I.e., in JavaScript/TypeScript case, an object without any functions defined on it, which is what can be communicated between different threads, processes, or distributed servers.
